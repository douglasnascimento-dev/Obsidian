...
> [!banner-icon] :LiBookMarked: ES45C
# :LiClipboard: **Atividades** | Qualidade de Software 
...
# :LiCalendarCheck2: **Registro de Aulas** | Qualidade de Software 
...
## :LiCalendarRange: Rastreamento de Presença | Qualidade de Software
...
```
# :LiBook: **Anotações** | Qualidade de Software
...
# :LiChartPie: **Avaliações** | Qualidade de Software
...
let tabelaDados = sortedPagesArray.map(p => {
    ...
    return [`[[${p.file.name}]] | ${p.Referência === null ? " " : p.Referência }`, p.Nota, (p.Peso).toFixed(2)];
});
